export function FileApprovalPreview({ arguments: raw }: { arguments: string }) {
  const plan = JSON.parse(raw) as {
    ...
    content?: string;
    before?: string;
    changes?: { target: string; before: string; after: string; beforeType?: string; afterType?: string }[];
    format?: string;
  ...
  return (
    <div className="file-approval-preview">
      <div className="file-destination">
        <span>{plan.action === 'read' ? 'Read document' : plan.action === 'edit' ? 'Update document' : 'Save new document'}</span>
        <p>{plan.path}</p>
      </div>
      {plan.action === 'read' ? (
        ...
        <>
          <p>{plan.changes ? 'Review each proposed change below. Unchanged document parts are preserved.' : 'Review the complete replacement below.'} Changes are saved only when you choose Apply changes. If the file changes in the meantime, Dext will ask again.</p>
          {plan.format === 'xlsx' && <p>Formulas are preserved. Excel will recalculate them when the workbook opens.</p>}
          {plan.changes ? plan.changes.map(change => (
            <section className="file-change" key={change.target} aria-label={change.target}>
              ...
              <div className="file-edit-comparison">
                <section><h4>Current content{change.beforeType && ` · ${change.beforeType}`}</h4><pre className="document-preview">{change.before || '(Empty)'}</pre></section>
                <section><h4>Proposed content{change.afterType && ` · ${change.afterType}`}</h4><pre className="document-preview">{change.after || '(Empty)'}</pre></section>
              </div>
            </section>
          )) : (
          <div className="file-edit-comparison">
