...
export class LocalCapabilities {
  ...
  async execute(activity: Activity, tool: any, signal: AbortSignal) {
    ...
    try {
      ...
      if (tool.name === 'mcp_bridge') {
        ...
        if (plan.action === 'read' && imageExtensions.includes(plan.format) && activity.provider !== 'openai') {
          const response = await fetch(`${endpoint(activity.ollamaUrl)}/api/show`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: activity.model }), signal: AbortSignal.any([signal, AbortSignal.timeout(10_000)]), redirect: 'error' });
          if (!response.ok) throw new Error('Could not check image support for the selected model.');
          const info = await response.json();
          if (!Array.isArray(info.capabilities) || !info.capabilities.includes('vision')) throw new Error('The selected model cannot read images. Choose a vision-capable model and try again.');
        }
  ...
  preparePlan(tool: any) { return prepareWorkPlan(tool.arguments, '', this.store.state, this.files); }
  async decideRuntimeApproval(activity: Activity, event: any, signal: AbortSignal) {
    ...
    const approved = await this.approval(activity, 'mcp', native ? `${plan.connection.name} · Create agent token and ${plan.tool.name}` : `${plan.connection.name} · ${plan.tool.name}`, JSON.stringify({ server: plan.connection.name, tool: plan.tool.name, arguments: plan.args, ...(native ? { token: { operations: native.operations, expiresIn: FUSED_TOKEN_LIFETIME, version: native.server.version, endpoint: plan.connection.url, scope: 'Token covers allowed operations across versions; Dext uses only this version endpoint.', purpose: plan.tool.name === 'connect' ? 'Create a scoped token and discover tools. No service action will run.' : `Create a scoped token, then run ${plan.tool.name} with these arguments.` } } : {}) }, null, 2), signal, event.approval.id, covered);
    return approved;
  }
  ...
  release(id: string) { this.mcp?.release(id); }
}
