...
export class LocalCapabilities {
  ...
  async decideRuntimeApproval(activity: Activity, event: any, signal: AbortSignal) {
    ...
    const approved = await this.approval(activity, 'mcp', native ? `${plan.connection.name} · Create agent token and ${plan.tool.name}` : `${plan.connection.name} · ${plan.tool.name}`, JSON.stringify({ server: plan.connection.name, tool: plan.tool.name, arguments: plan.args, ...(native ? { token: { operations: native.operations.length ? native.operations : ['*'], expiresIn: FUSED_TOKEN_LIFETIME, version: native.server.version, endpoint: plan.connection.url, scope: 'Token covers allowed operations across versions; Dext uses only this version endpoint.', purpose: `Create a scoped token, then run ${plan.tool.name} with these arguments.` } } : {}) }, null, 2), signal, event.approval.id, covered);
    return approved;
  }
