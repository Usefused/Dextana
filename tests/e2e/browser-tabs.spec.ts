...
test('tab selection swaps the native page without closing the pane or stacking old views', async ({ workspace }) => {
  test.setTimeout(120_000);
  const site = createServer((req, res) => {
    const blue = req.url === '/blue';
    res.setHeader('Content-Type', 'text/html');
    res.end(`<title>${blue ? 'Blue' : 'Red'} page</title><body style="margin:0;background:${blue ? '#0000ff' : '#ff0000'};height:100vh"><h1>${blue ? 'Blue' : 'Red'} page</h1></body>`);
  });
  ...
  const work = await workspace((body, res) => {
    ...
    else reply(body, res, '', [{ function: { name: 'browser', arguments: { action: 'open', url: base + '/red' } } }]);
    return true;
  });
  async function visiblePage(path: string, color: 'red' | 'blue') {
    ...
    await expect.poll(() => work.app().evaluate(async ({ BrowserWindow }, base) => {
      const host = BrowserWindow.getAllWindows()[0];
      const views = host.contentView.children.filter((view: any) => view.webContents?.getURL().startsWith(base)) as import('electron').WebContentsView[];
      if (views.length !== 1) return { count: views.length };
      const view = views[0], bounds = view.getBounds();
      const pixel = (await view.webContents.capturePage({ x: Math.floor(bounds.width / 2), y: Math.floor(bounds.height / 2), width: 1, height: 1 })).toBitmap();
      // Native captures are color-managed on macOS; check the dominant channel.
      const color = pixel[2] > pixel[0] + 100 && pixel[2] > pixel[1] + 100 ? 'red' : pixel[0] > pixel[2] + 100 && pixel[0] > pixel[1] + 100 ? 'blue' : 'other';
      return { url: view.webContents.getURL(), visible: view.getVisible(), topmost: host.contentView.children.at(-1) === view, color };
    }, base)).toEqual({ url: base + path, visible: true, topmost: true, color });
  }
  try {
    ...
    await work.page.evaluate(() => {
      ...
      (window as any).__stopTabViews = window.dextana.subscribe(state => (window as any).__tabViews.push(state.browser?.url ?? null));
    });
    for (const path of ['/red', '/blue', '/red']) {
      await work.page.getByRole('tab', { name: new RegExp(path === '/red' ? 'Red page' : 'Blue page') }).click();
