...
export async function* browserTabs(
  ...
  try {
    ...
    const selectedBounds = (await tabs
    ...
    const savedActivityId = await work.page.evaluate(
      async () =>
        (await window.dextana.snapshot()).activities.find(
          (activity) => activity.title === 'Prepare two browser tabs',
        )!.id,
    );
    ...
    await work.page.locator('.session-access-menu > summary').click();
