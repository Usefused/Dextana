...
export async function* fusedConnections(
  ...
  try {
    ...
    integrationId = await work.page.evaluate(async (url) => {
      const id = await window.dextana.saveFused({
        name: 'Fused',
        enabled: true,
        url,
        token: 'local-test-token',
      });
      await window.dextana.saveFused({
        name: 'Support',
        enabled: true,
        url: url.replace('/mcp', '/support'),
        token: 'other-test-token',
      });
      return id;
    }, url);
    await expect(
      work.page.getByRole('region', { name: 'Fused integration Support', exact: true }),
    ).toBeVisible();
