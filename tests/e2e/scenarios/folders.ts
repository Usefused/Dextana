...
export async function* folders(
  ...
  try {
    ...
    await expect(
      folder.getByRole('button', { name: 'New conversation inside Projects', exact: true }),
    ).toBeVisible();
    ...
    await expect(
      restoredFolder.getByRole('button', { name: 'New conversation inside Projects', exact: true }),
    ).toBeVisible();
    ...
    expect(
      state.activities.find((activity) => activity.title === 'Prepare project notes')?.folderId,
    ).toBeUndefined();
  } finally {
    await work.close();
  }
