...
test('agent edits show exact before/after, require consent even with session access, and reject stale files', async ({ workspace }) => {
  ...
  const work = await workspace((body, res) => {
    ...
    if (!results.length) reply(body, res, '', [{ function: { name: 'files', arguments: { action: 'read', path } } }]);
    else if (results.length === 1) {
      const result = JSON.parse(results[0].content);
      expect(result.revision).toMatch(/^[a-f0-9]{64}$/);
      reply(body, res, '', [{ function: { name: 'files', arguments: { action: 'edit', path, content: 'Budget: 300\nKeep these notes.\n', expected_revision: result.revision } } }]);
  ...
  try {
    ...
    await expect(gate.getByRole('region', { name: 'Current file content' })).toContainText('Budget: 250');
    await expect(gate.getByRole('region', { name: 'Proposed file content' })).toContainText('Budget: 300');
...
for (const format of ['docx', 'xlsx'] as const) {
  test(`agent proposes reviewed ${format} changes and preserves original on denial`, async ({ workspace }) => {
    test.setTimeout(120_000);
    let path = '';
    const work = await workspace((body, res) => {
      const last = body.messages.findLastIndex((message: any) => message.role === 'user');
      ...
      if (!results.length) reply(body, res, '', [{ function: { name: 'files', arguments: { action: 'read', path } } }]);
        ...
        const result = JSON.parse(results[0].content);
        expect(result.revision).toMatch(/^[a-f0-9]{64}$/);
        if (format === 'docx') expect(result.paragraphs[0]).toMatchObject({ paragraph: 1, text: 'Original budget: 250' });
        const edits = format === 'docx' ? [{ paragraph: 1, find: '250', replace: '300' }] : [{ sheet: 'Budget', cell: 'B2', value: 300 }];
        reply(body, res, '', [{ function: { name: 'files', arguments: { action: 'edit', path, expected_revision: result.revision, edits_json: JSON.stringify(edits) } } }]);
      } else reply(body, res, 'Edit result: ' + results.at(-1).content);
      return true;
    });
    try {
      ...
      if (format === 'docx') {
        ...
        workbook.addWorksheet('Budget').addRows([['Item', 'Amount'], ['Travel', 250]]);
      ...
      const change = gate.getByRole('region', { name: format === 'docx' ? 'Paragraph 1' : 'Budget!B2', exact: true });
