...
test('stream follows content, respects manual scrolling, and resumes at latest', async ({
    ...
    (window as any).dextana = {
      ...
      start: async (input: any) => {
        if (snapshot.activities[0].status === 'completed') {
          snapshot.activities[0].messages.push({ id: 'follow-up', role: 'user', content: input.prompt }, { id: 'next-reply', role: 'assistant', content: '' });
  ...
  expect(contextBounds.x + contextBounds.width).toBeLessThanOrEqual(mainBounds.x + mainBounds.width);
  ...
  await expect(page.locator('.message.user').last()).toContainText('A new message after reading earlier output');
  await expect.poll(gap).toBeLessThan(3);
...
test('renders Markdown and streamed results without exposing display payloads', async ({ page }) => {
  const markdown =
