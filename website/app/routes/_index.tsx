...
function PlatformDownload() {
  ...
  return <div className="platform-picker" role="group" aria-label="Download Dextana by Fused">
    ...
    <div id="platform-details" className="platform-details" aria-live="polite">
      {selected ? <><strong>Latest alpha · {selected.architecture}</strong><span>{selected.format}. {selected.instructions}</span></> : <span>Select your operating system to show its download button.</span>}
    </div>
    {selected && <a className="button platform-cta" href={selected.url}>Download for {selected.name} <span aria-hidden="true">↓</span></a>}
