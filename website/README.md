...
The dropdown starts without a platform selected. Selecting macOS, Windows, or Linux reveals the architecture, version, archive size, installation instructions, and a “Download for [platform]” button. The buttons point directly to the existing installer package downloads, not to a workflow page. Each ZIP contains the packaged desktop binaries (DMG for macOS Apple silicon, EXE for Windows x64, AppImage for Linux x64). No-JavaScript links target the same three packages.

The current links were verified against successful manual packaging run `34288516486`, commit `0440ad31e98c6c443cae7a7bf73c1206eca7170c`, version `0.1.0`. The repository is private, so users still need GitHub sign-in and repository access. These are GitHub package ZIP downloads, not separately hosted individual installer files. They expire on 22 September 2026; the interactive selector hides expired download buttons. Update `downloads.platforms` in `app/content.ts` with verified replacement package URLs, architectures, sizes, and expiry dates, then rebuild/redeploy. When durable release assets are published, replace these URLs with their direct binary download URLs. Do not expose GitHub tokens or short-lived signed storage URLs in the page. The local `release/` files are not uploaded by this website.
