...
The agent can invoke `browser(action="disconnect_user")` when you ask it to end browser access. Dext revokes the shared external-browser connection for every chat, rejects pending browser actions, and leaves the browser's tabs open. The action uses the normal browser approval policy.

In a chat, open **Chat settings**, choose **Use my browser**, then **Create connection code**. Copy the code, open **Dextana Browser** in Chrome or Edge, paste it, and choose **Activate browser access**. By default this grants access to all regular web tabs in that browser profile, across windows, including tabs opened later. Dext can open its own task tabs. Other chats reuse this activation through their normal browser approval flow. Keep Dextana open.
...
The in-app tab strip lets you view saved in-app pages while that browser pane is active. Viewing a page does not switch the agent's browser target, reset a Chrome connection, or authorize a pending action against a different tab. Use **Use Dext’s in-app browser** in the browser connection dialog to change where the agent works.
...
- Track downloads initiated from controlled tabs used by the chat, including attachments. The browser's normal download settings choose the destination; completed receipts expose the saved path so the agent can request separate permission to read the file.
...
This mode currently excludes embedded-frame controls, coordinate clicks, double-clicks and clipboard access. Browser-managed pages, private windows and non-HTTP(S) URLs cannot be attached. PDFs may require screenshot navigation before downloading because their viewer controls are not ordinary page elements. Chrome/Edge support is based on Chromium APIs; Firefox and Safari adapters are not implemented. Windows and Linux have not received live acceptance testing for this feature.
