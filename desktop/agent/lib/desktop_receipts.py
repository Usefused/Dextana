...
def desktop_result(operation, value, refs):
    ...
    if operation == 'computer.status':
        result = fields(value, 'enabled', 'permissions', 'instruction')
        if value.get('active'):
            result['active'] = fields(value['active'], 'state')
        return result
    if operation == 'computer.observe':
        return dict(snapshotId=refs.public('Observation', value['snapshotId']),
                    **fields(value, 'application', 'window', 'elements', 'returned_element_count', 'image'))
