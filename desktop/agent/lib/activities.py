...
class Activities:
    ...
    def configure(self, data):
        ...
        if 'mcpCatalog' in data:
            self.state['mcpCatalog'] = copy.deepcopy(data['mcpCatalog'])
        if 'settings' in data:
    ...
    def remind(self, activity_id, text, occurrence, schedule_id, overdue=False, due_at=None):
        ...
        if not any(message['id'] == occurrence for message in activity['messages']):
            activity['messages'].append(dict(id=occurrence, createdAt=now(), role='assistant', content='Reminder: ' + text,
    ...
    def edit_message(self, data):
        ...
        try:
            ...
            result = self.start(dict(activityId=activity['id'], prompt=data.get('prompt'),
                ...
                edited=dict(id=message['id'], createdAt=message.get('createdAt'), editedAt=now(), **({'replyToMessageId': message['replyToMessageId']} if message.get('replyToMessageId') else {})))
    ...
    def _start(self, data, parent=None, approved=None, queued_id=None, internal=False, edited=None):
        ...
        if reasoning not in ('default', 'off', 'on', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max'):
            raise ValueError('Choose a valid reasoning setting.')
        queued = dict(id=uid(), createdAt=now(), prompt=prompt.strip(), model=model, reasoning=reasoning, mode=data.get('mode', (activity or {}).get('mode', 'work')), files=data.get('files', []))
        if reply_to is not None:
    ...
    def turn(self, activity, data):
        ...
        message = dict(id=data['id'], eventStart=len(activity['events']), createdAt=data.get('createdAt') or now(), role='user', content=data['prompt'], model=data['model'], reasoning=activity['reasoning'], mode=activity['mode'], files=data.get('files', []))
    ...
    def mcp_availability(self, activity):
        catalog = self.state.get('mcpCatalog')
        if catalog is None or activity.get('mcpCatalogSeen') == catalog:
            return ''
        activity['mcpCatalogSeen'] = copy.deepcopy(catalog)
        return ('MCP connector configuration changed. Discard earlier cached availability results. '
                f'The current catalog has {len(catalog)} enabled connectors. '
                'Call mcp(action="list") to refresh exact tools and schemas before using connected services '
                'or claiming tools are unavailable. Existing execution approvals still apply.\n')
    ...
    async def run(self, activity):
        ...
        try:
            async with asyncio.timeout(IDLE_SECONDS) as deadline, httpx.AsyncClient(base_url=base, headers=headers, trust_env=False) as client:
                ...
                prompt = self.mcp_availability(activity) + '\n' + prompt
                await self.memory.before(activity)
                activity['status'] = 'running'
                message = dict(id=uid(), createdAt=now(), role='assistant', content='', model=activity['model'], reasoning=activity.get('reasoning', 'default'))
                activity['messages'].append(message)
                self.commit()
                response_id = None
