...
def execution_error(error):
    if context_limit_error(error):
        return ('The selected model rejected the conversation size even after Dext tried to compact it. '
                'Start a new chat with a short summary or choose a model with a larger usable context window.')
