...
def test_computer_session_excludes_private_ids_but_observations_still_route(boundary):
    status = desktop(boundary, 'computer.status', dict(enabled=True,
        permissions=dict(accessibility=True, screenRecording=True),
        instruction='Each action needs approval',
        active=dict(id='private-session', activityId='private-chat', state='ready')))
    assert status == dict(enabled=True,
                         permissions=dict(accessibility=True, screenRecording=True),
                         instruction='Each action needs approval', active=dict(state='ready'))
    observed = desktop(boundary, 'computer.observe', dict(snapshotId='private-snapshot',
        snapshot_id='private-snapshot', pid=42, futureMetadata=True,
        application='Editor', window='Invoice',
        elements=[dict(element_index=3, label='Business ID 123')], returned_element_count=1,
        image={'type': 'image', 'data': 'bytes'}))
    assert set(observed) == {'snapshotId', 'application', 'window', 'elements', 'returned_element_count', 'image'}
    args = boundary.arguments('desktop_bridge', dict(phase='prepare', operation='computer.act',
        arguments_json=json.dumps(dict(snapshotId=observed['snapshotId'],
                                       elementIndex=3, text='Business ID 123'))))
    assert json.loads(args['arguments_json']) == dict(snapshotId='private-snapshot',
                                                     elementIndex=3, text='Business ID 123')
