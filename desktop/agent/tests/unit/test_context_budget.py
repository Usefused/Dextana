...
def test_provider_limit_error_retries_once_with_forced_compaction(agent):
    from harnest.lib import ollama
    calls = []
    class Delegate:
        async def acompletion(self, **arguments):
            calls.append((arguments, ollama._forced_context_bytes.get()))
            if len(calls) == 1:
                raise ValueError('Range of input length should be [1, 258048]')
            return 'recovered'
    async def run():
        result = await ollama.ContextRecoveryClient(Delegate()).acompletion(messages=[])
        assert result == 'recovered'
        assert calls == [({'messages': []}, None), ({'messages': []}, 128_000)]
        assert ollama._forced_context_bytes.get() is None
    asyncio.run(run())


def test_desktop_model_installs_context_recovery_client(agent):
    from harnest.lib import ollama
    built = ollama.desktop_model().build()
    assert isinstance(built.llm_client, ollama.ContextRecoveryClient)


def test_stream_provider_limit_retries_only_before_output(agent):
    from harnest.lib import ollama
    class Stream:
        ...
        async def __anext__(self):
            ...
            return value
    async def run():
        recovered = ollama.ContextRecoveryStream(
            Stream([ValueError('maximum context length exceeded')]),
            lambda: asyncio.sleep(0, result=Stream(['ok'])),
        )
        assert await recovered.__anext__() == 'ok'
        partial = ollama.ContextRecoveryStream(
            Stream(['partial', ValueError('maximum context length exceeded')]),
            lambda: asyncio.sleep(0, result=Stream(['duplicate'])),
        )
        assert await partial.__anext__() == 'partial'
