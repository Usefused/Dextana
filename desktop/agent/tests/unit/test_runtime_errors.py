...
def test_context_rejection_is_explained_without_gateway_details(agent):
    from harnest.lib.runtime_errors import execution_error
    error = ValueError('litellm.BadRequestError: Range of input length should be [1, 258048] ref: private')
    message = execution_error(error)
    assert 'tried to compact' in message
    assert '258048' not in message
