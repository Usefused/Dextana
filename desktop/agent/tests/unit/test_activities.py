...
def test_backend_queue_steer_and_restart(agent, tmp_path):
    ...
    async def check():
        ...
        assert datetime.fromisoformat(activity['messages'][0]['createdAt']).tzinfo is not None
        assert activity['messages'][0]['eventStart'] == 0
        activity['events'].append('Original action')
        ...
        assert activity['messages'][-1]['eventStart'] == 1
        ...
        assert restored.get(activity_id)['messages'] == activity['messages']
        backend.configure({'mcpCatalog': []})
        assert '0 enabled connectors' in backend.mcp_availability(activity)
        assert backend.mcp_availability(activity) == ''
        backend.configure({'mcpCatalog': [{'id': 'enabled-now', 'tools': [{'name': 'read_tasks'}]}]})
        assert 'configuration changed' in backend.mcp_availability(activity)
        assert backend.mcp_availability(activity) == ''
        backend.configure({'mcpCatalog': []})
        assert '0 enabled connectors' in backend.mcp_availability(activity)
