...
import { TeachControlBar, TeachDexView } from './TeachDex';
import { MCPAuthentication } from './MCPAuthentication';
import type { TaughtSkill, TaughtSkillDraft } from '../shared/teach-dex';
import './teach-dex.css';
...
const settingsPages = {
  integrations: {
    title: 'Integrations',
    description:
      'Explore supported apps, connect your accounts and manage your Dext Integrations subscription.',
  },
  computer: {
    title: 'Computer use',
    description: 'Configure app control, per-action approval and macOS permissions.',
  },
  browser: {
    title: 'Browser use',
    description: 'Set global browser defaults and individual chat overrides.',
  },
  background: {
    title: 'Background service',
    description: 'Choose how Dextana runs when you close its window.',
  },
  appearance: {
    icon: 'M10 2a8 8 0 1 0 8 8c0-1.5-1.5-2-3-2h-2a2 2 0 0 1-2-2c0-1 1-2 1-3 0-.7-1-1-2-1Z M6 6h.01 M4.5 10h.01 M7 14h.01',
    title: 'Appearance',
    description: 'Make Dextana feel at home on your desktop.',
  },
  models: {
    icon: 'M6 5h8a1 1 0 0 1 1 1v8a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1Z M8 8h4v4H8Z M7 2v3m6-3v3M7 15v3m6-3v3M2 7h3m-3 6h3m10-6h3m-3 6h3',
    title: 'Models',
    description: 'Connect Ollama or an OpenAI-compatible model provider.',
  },
  ...
  mcp: {
    icon: 'M7 2v4m6-4v4M5 6h10v3a5 5 0 0 1-10 0V6Z M10 14v4',
    title: 'Connectors',
    description: 'Manage the tools and services your assistant can use.',
  },
};
function App() {
  ...
  const [workspaceView, setWorkspaceView] = useState<'activities' | 'cron' | 'desktop' | 'teach'>(
    'activities',
  );
  const [desktopView, setDesktopView] = useState<DesktopView>('time');
  const [contextHidden, setContextHidden] = useState(false);
  const [sidebarCollapsed, setSidebarCollapsed] = useState(
    () => localStorage.getItem('dext.sidebarCollapsed') === 'true',
  );
  useEffect(() => {
    localStorage.setItem('dext.sidebarCollapsed', String(sidebarCollapsed));
  }, [sidebarCollapsed]);
  ...
  useLayoutEffect(() => {
    ...
    void window.dextana.selectActivity(
      settingsOpen || workspaceView !== 'activities' ? undefined : selectedId,
    );
  }, [selectedId, settingsOpen, workspaceView]);
  useLayoutEffect(() => {
    const view =
      settingsOpen || workspaceView === 'teach'
        ? undefined
        : workspaceView === 'desktop'
          ...
          : workspaceView === 'activities'
            ? {
                page: 'chat',
                target: selectedId
                  ? { kind: 'activity' as const, activityId: selectedId }
                  : undefined,
              }
            : { page: workspaceView };
    void window.dextana
  ...
  useEffect(
    () =>
      window.dextana.onOpenActivity((id) => {
        setSelectedId(id);
  ...
  useEffect(
    () =>
      window.dextana.onOpenDesktop((id) => {
        if (id && id === snapshot?.desktopComputer?.active?.id) {
          openSettings('computer');
          return;
        }
        setSelectedDesktopResource(id);
  ...
  useEffect(() => {
    setMode(activity?.mode ?? 'work');
  }, [selectedId, activity?.mode]);
  useEffect(() => {
    if (activity) {
      setModel(
        snapshot?.settings.models.includes(activity.modelSelection?.model ?? activity.model)
          ? (activity.modelSelection?.model ?? activity.model)
          : '',
      );
      setReasoning(activity.modelSelection?.reasoning ?? activity.reasoning ?? 'default');
    }
  }, [selectedId]);
  ...
  async function launchTaughtSkill(skill: TaughtSkill, inputs: Record<string, string>) {
    if (!model) throw new Error('Connect and select a model before running a skill.');
    const result = await window.dextana.teach({ action: 'run_prompt', skill, inputs });
    if (!('prompt' in result)) throw new Error('Could not prepare this skill run.');
    const activityId = await window.dextana.start({
      mode: 'work',
  ...
  async function tryDraftSkill(skill: TaughtSkillDraft, inputs: Record<string, string>) {
    const saved = await window.dextana.teach({ action: 'save', draft: skill, status: 'draft' });
    if (!('version' in saved)) throw new Error('Could not save the skill draft.');
    await launchTaughtSkill(saved, inputs);
  }
  useEffect(() => {
  ...
  async function send() {
    ...
    setError('');
    try {
      const id = await window.dextana.start({
        mode,
        prompt,
        model,
        reasoning,
        ...
        folderId: selectedId ? undefined : draftFolderId,
      });
      setSelectedId(id);
  ...
  function persistSelection(nextModel: string, nextReasoning: Reasoning) {
    ...
    selectionSave.current = selectionSave.current
      .catch(() => {})
      .then(() => window.dextana.selectModel(id, nextModel, nextReasoning))
  ...
  async function resume() {
    ...
    setError('');
    try {
      await window.dextana.resume(activity.id);
      setFollowRequest((value) => value + 1);
    } catch (error) {
  ...
  const composer = (
    <div className="composer">
      ...
      {!!attachments.length && (
        <div className="attachments" role="region" aria-label="Attached files">
          {attachments.map((path) => (
            <span key={path} title={path}>
              {path.split(/[\\/]/).at(-1)}{' '}
              <Button
                variant="layout"
                aria-label={`Remove attachment ${path.split(/[\\/]/).at(-1)}`}
                onClick={() => setAttachments((items) => items.filter((item) => item !== path))}
              >
                ×
              </Button>
            </span>
      ...
      {activity && (
        <MessageQueue
          ...
          items={activity.queue ?? []}
          running={!!running}
          disabled={steering || sending || !!activity.archived || !!activity.parentId}
          failed={setError}
          steer={(messageId) => {
            setSteering(true);
            setError('');
            void window.dextana
              .steer(activity.id, messageId)
              .then(() => {
                setFollowRequest((value) => value + 1);
              })
              .catch((error) => setError(error.message))
      ...
      <div className="composer-footer">
        <Select
          className="activity-mode"
          title={
            mode === 'plan'
              ? 'Draft a plan for your approval before taking action'
        ...
        <Button
          ...
          onClick={async () => {
            try {
              ...
              setAttachments((items) => [...new Set([...items, ...paths])].slice(0, 20));
            } catch (error) {
              setError((error as Error).message);
            }
          }}
        >
          <svg
            ...
            aria-hidden="true"
          >
            <path d="M6 2.5h6l4 4v10a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1v-13a1 1 0 0 1 1-1Z" />
            <path d="M12 2.5v4h4M8 10h5M8 13h5" />
          </svg>
          <span>Files</span>
        </Button>
        <ModelSelector
          key={`${snapshot?.settings.provider}:${snapshot?.settings.connectionId}`}
          models={snapshot?.settings.models ?? []}
          model={model}
          reasoning={reasoning}
          url={snapshot?.settings.ollamaUrl ?? ''}
          disabled={sending}
          changeModel={(value) => {
            setModel(value);
            setReasoning('default');
            persistSelection(value, 'default');
          }}
          changeReasoning={(value) => {
            setReasoning(value);
            persistSelection(model, value);
          }}
        />
        <Button
          variant="layout"
          ...
          onClick={
            canResume
              ...
              : running && !prompt.trim()
                ? () => {
                    void window.dextana.cancel(activity!.id).catch((e) => setError(e.message));
                  }
                : send
          }
          disabled={
            ...
            (!canResume &&
              !(running && !prompt.trim()) &&
              (!model || !snapshot?.settings.models.includes(model) || !prompt.trim()))
          }
        >
  ...
  useEffect(
    () => window.dextana.onOpenSettings?.((page) => openSettings(page ?? 'models')),
    [snapshot, settingsOpen],
  );
  async function connect() {
    ...
    try {
      ...
      if (!Array.isArray(catalog.vision)) {
        throw new Error(
          'Restart Dextana to finish updating model discovery, then connect again to load image interpreter models.',
        );
      }
  ...
  return (
    <div
      className={
        (sidebarCollapsed && !settingsOpen ? 'sidebar-collapsed ' : '') +
        (settingsOpen
          ? 'shell settings-shell'
          : snapshot?.browser && activity && workspaceView === 'activities'
            ? 'shell with-browser'
            : 'shell')
      }
    >
        ...
        {settingsOpen ? (
          <>
            ...
            <nav className="settings-nav" aria-label="Settings sections">
              {(
                [
                  ['Preferences', ['appearance', 'models', 'usage']],
                  ['Customize', ['skills', 'integrations', 'mcp']],
                  ['Automation', ['computer', 'browser', 'background']],
                ] as const
              ).map(([group, ids]) => {
                const matches = ids.filter((id) =>
                  (settingsPages[id].title + ' ' + settingsPages[id].description)
                    .toLowerCase()
              ...
              {!Object.values(settingsPages).some((page) =>
                (page.title + ' ' + page.description)
                  .toLowerCase()
                  .includes(settingsSearch.toLowerCase()),
              ) && <p className="settings-caption">No matching settings</p>}
            </nav>
          </>
        ) : (
          <>
            <div className="new-work-row">
              <Button
                variant="layout"
            ...
            <ActivityFolders
              view={workspaceView === 'teach' ? 'activities' : workspaceView}
              ...
              renderActivity={(a) => (
                <div className="activity-row" key={a.id}>
                  ...
                  >
                    <span
                      className={`activity-dot ${a.approval ? 'needs-approval' : a.status}`}
                      title={a.approval ? 'Needs approval' : undefined}
                    />
                  ...
                  <Button
                    ...
                    aria-label={`Archive ${displayTitle(a.title)}`}
                    title="Archive activity"
                    disabled={archiving}
                    onClick={() => {
                      void archiveChat(a.id);
                    }}
                  >
                    <svg width="15" height="15" viewBox="0 0 16 16" fill="none" aria-hidden="true">
                      <path
                        d="M3 6v7h10V6M2 3h12v3H2zM6 9h4"
                        stroke="currentColor"
                        strokeWidth="1.2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
      ...
      >
        <header>
          {!settingsOpen && (
            <IconButton
              ...
              label={sidebarCollapsed ? 'Show chat sidebar' : 'Collapse chat sidebar'}
          ...
          <span className="session-heading">
            <span className="session-name">
              ...
              {settingsOpen
                ? `Settings / ${settingsPages[settingsPage].title}`
                : workspaceView === 'cron'
                  ? 'Scheduled jobs'
                  : workspaceView === 'desktop'
                    ? 'Desktop'
                    : workspaceView === 'teach'
                      ...
                      : activity
                        ...
                        : draftFolderId
                          ? `New chat · ${snapshot?.folders?.find((folder) => folder.id === draftFolderId)?.name ?? 'Workspace'}`
                          : 'New activity'}
            </span>
          </span>
          {activity && !settingsOpen && workspaceView === 'activities' ? (
            <div className="chat-header-actions">
              <SessionSettings
                key={activity.id}
                activity={activity}
                browserAutoAllow={snapshot?.browserPreferences?.autoAllow === true}
                showContext={!contextHidden}
                toggleContext={() => setContextHidden((value) => !value)}
                folderControl={
                  <FolderPicker
                    activity={activity}
                    folders={snapshot?.folders ?? []}
                ...
                browserControl={(close) => (
                  <UserBrowserButton
                    key={`user-${activity.id}`}
                    activityId={activity.id}
                    state={snapshot?.userBrowsers?.find(
                      (state) => state.activityId === activity.id,
                    )}
                    failed={setError}
                    opened={close}
                  />
        ...
        <UserBrowserStatus
          state={snapshot?.userBrowsers?.find((state) => state.activityId === activity?.id)}
          failed={setError}
        />
        ...
        {workspaceView === 'desktop' && !settingsOpen && snapshot ? (
          <DesktopWorkspace
            view={desktopView}
            setView={setDesktopView}
            close={() => setWorkspaceView('activities')}
            snapshot={snapshot}
            focusedResource={selectedDesktopResource}
            openActivity={(id) => {
              setSelectedId(id);
              setWorkspaceView('activities');
            }}
          />
        ) : workspaceView === 'cron' && !settingsOpen && snapshot ? (
          <CronView
            snapshot={snapshot}
            openActivity={(id) => {
              setSelectedId(id);
              setWorkspaceView('activities');
              const run = snapshot.activities.find((a) => a.id === id);
              if (run) setModel(run.model);
              setPrompt('');
              setAttachments([]);
            }}
          />
          ...
          <div className="settings-scroll">
            <section className="settings-panel">
              <PageHeader
                title={settingsPages[settingsPage].title}
                description={settingsPages[settingsPage].description}
              />
              {settingsPage === 'computer' && snapshot && (
                <ComputerUseSettings snapshot={snapshot.desktopComputer ?? { enabled: false }} />
              )}
              {settingsPage === 'integrations' && <IntegrationsSettings />}
              {settingsPage === 'browser' && snapshot && (
                <BrowserUseSettings
                  defaultAutoAllow={snapshot.browserPreferences?.autoAllow === true}
              ...
              {settingsPage === 'background' && (
                <BackgroundServiceSettings enabled={snapshot?.desktopBackground !== false} />
              )}
              {settingsPage === 'skills' && (
                <SkillsSettings
                  teach={openTeach}
                  key={snapshot?.teaching?.run?.state ?? 'idle'}
                  runTaught={launchTaughtSkill}
                  editTaught={(skill) => {
                    void window.dextana
                      .teach({ action: 'review', skill })
                      .then(openTeach)
                      .catch((error) => setError(error.message));
                  }}
                />
              ...
              {settingsPage === 'appearance' && (
                <AppearanceSettings
                  ...
                  saved={async () => setSnapshot(await window.dextana.snapshot())}
                />
              )}
              <div hidden={settingsPage !== 'models'}>
                <FormSection className="model-settings-form" aria-label="Model connection">
                  <Field
                    ...
                    hint={
                      draft.provider === 'openai'
                        ? 'Use OpenRouter or another compatible chat-completions endpoint.'
                        : 'Use the models installed on your Ollama server.'
                    }
                  >
                    {(props) => (
                      <Select
                  ...
                  <Field
                    variant="card"
                    label={draft.provider === 'openai' ? 'Base URL' : 'Ollama address'}
                    hint="Where Dextana connects to your models."
                  >
                    {(props) => (
                      <TextInput
                        {...props}
                        ...
                        onChange={(e) => {
                          setDraft({
                            ...
                            models: [],
                            imageInterpreterModel: undefined,
                            embeddingModel: undefined,
                            embeddingDimensions: undefined,
                            memoryError: undefined,
                            authMode: undefined,
                            hasCustomAuth: undefined,
                          });
                          setApiKey(undefined);
                          setAuthDraft(undefined);
                          setConnected(false);
                  ...
                  {draft.provider === 'openai' && (
                    <>
                      <ModelAuthSettings
                        key={`auth:${draft.provider}:${draft.ollamaUrl}`}
                        settings={draft}
                        draft={authDraft}
                        disabled={busy}
                        change={(value) => {
                      ...
                      {snapshot?.settings.hasApiKey && (
                        <div className="dx-actions">
                          <Button
                            ...
                            onClick={() => setApiKey('')}
                          >
                            {apiKey === '' ? 'Key will be removed on save' : 'Remove saved key'}
                          </Button>
                        </div>
                      )}
                    </>
                  ...
                  <div className="dx-actions">
                    ...
                    {connected && (
                      <span role="status" className="settings-caption">
                        {draft.models.length
                  ...
                  {draft.provider === 'openai' && (
                    <div className="model-manual-entry">
                      <Field label="Model ID">
                        {(props) => (
                          <TextInput
                            ...
                            value={manualModel}
                            disabled={busy}
                            placeholder="Or enter a chat model ID manually"
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (!isChatModel({ id: manualModel.trim() })) {
                            setError(
                              'Choose a chat model. Embedding and reranking models are not supported.',
                            );
                            return;
                          }
                          setDraft({
                            ...draft,
                            models: [...new Set([...draft.models, manualModel.trim()])],
                          });
                          setManualModel('');
                          setConnected(true);
                        }}
                  ...
                  <ImageInterpreterSettings
                    key={`image:${draft.provider}:${draft.ollamaUrl}`}
                    settings={draft}
                    models={visionModels}
                    disabled={busy}
                    change={setDraft}
                  />
                  <MemorySettings
                    key={`${draft.provider}:${draft.ollamaUrl}`}
                    settings={draft}
                  ...
                  <div className="dx-form-actions">
                    <Button
                      ...
                      variant="primary"
                      disabled={!draft.models.length || busy}
                      onClick={save}
                    >
                      Save settings
                    </Button>
              ...
              <div hidden={settingsPage !== 'mcp'}>
                {snapshot && (
                  <MCPSettings
                    workspace={snapshot.fusedWorkspace}
                    connections={snapshot.mcpConnections ?? []}
                    account={snapshot.fusedAccount}
                    saved={async () => setSnapshot(await window.dextana.snapshot())}
                  />
                )}
                {!!snapshot?.fusedIntegrations?.length && (
                  <FusedSettings
                    integrations={(snapshot.fusedIntegrations ?? []).filter(
          ...
          <TeachDexView state={snapshot?.teaching} failed={setError} runSkill={tryDraftSkill} />
        ) : activity ? (
          <div className="conversation-layout">
            ...
            <ContextPanel
              key={activity.id}
              minimize={() => setContextHidden(true)}
              hidden={contextHidden}
              activity={activity}
              changed={async () => setSnapshot(await window.dextana.snapshot())}
            />
          </div>
        ) : (
          ...
          <div className="action-notice" role="status">
            ...
            {notice.archivedId && (
              <Button
                ...
                disabled={archiving}
                onClick={() => {
                  void undoArchive(notice.archivedId!);
                }}
              >
                Undo
              </Button>
        ...
        <footer>
          <span className="footer-dot" /> Yours by design · Local first{' '}
          <span>Built to do the work.</span>
        </footer>
      </main>
      {snapshot?.browser && activity && !settingsOpen && workspaceView === 'activities' && (
        <BrowserPanel browser={snapshot.browser} activities={snapshot.activities} />
      )}
      <TeachControlBar state={snapshot?.teaching} failed={setError} open={openTeach} />
      {snapshot?.mcpAuthentications?.[0] && (
        <MCPAuthentication
          key={snapshot.mcpAuthentications.at(-1)!.id}
          authentication={snapshot.mcpAuthentications.at(-1)!}
        />
      )}
    </div>
  );
