...
function ApprovalTarget({
  ...
  return (
    <ApprovalDetails
      ...
      desktop={approval.capability === 'desktop' || approval.capability === 'computer'}
    />
  );
}
...
function ApprovalFooter({
  ...
  const sessionAccess =
    (approval.source !== 'harnest' || ['mcp', 'computer'].includes(approval.capability)) &&
    approval.capability !== 'fileEdit';
  return (
    <>
      {sessionAccess && (
...
export function AutomaticAccess({ activity }: { activity: Activity }) {
  ...
  const [error, setError] = useState('');
  const allowed = (
    ['browser', 'mcp', 'fileRead', 'fileCreate', 'desktop', 'computer'] as const
  ).filter((capability) => activity.permissions?.[capability] === true);
