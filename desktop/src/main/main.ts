import { buildDevelopmentBackend } from './development-backend';
...
import { DextComputerCursor } from './desktop/cursor';
...
import type { TeachCommand } from '../shared/teach-dex';
import { TeachDex } from './teach-dex';
import { CuaTeachingCapture, FixtureTeachingCapture } from './teach-capture';
...
app
  ...
  .then(async () => {
    ...
    const modelConnections = new ModelConnections(
    ...
    if (store.state.settings.models.length && store.state.settings.provider !== 'openai') {
      try {
        ...
        /* Keep offline settings; refresh when Ollama becomes available. */
      }
    }
    const runtime = new Runtime(
      app.isPackaged
        ? join(process.resourcesPath, 'backend')
        : join(app.getAppPath(), '.build/backend'),
      app.getPath('userData'),
    );
    ...
    let teachDex: TeachDex | undefined;
    const publish = () => {
      ...
      void teachDex?.observeActivities(store.state.activities);
    };
    store.state.userBrowsers = [];
    const userBrowsers = new UserBrowser(
      `chrome-extension://${extensionId(extensionManifest.key)}`,
      (states) => {
    ...
    const browsers = new Browsers(
      ...
      },
      {
        ...
        defaultTab: (id) =>
          store.state.activities.find((activity) => activity.id === id)?.browser?.activeTabId,
        ...
        save: (id, tabs, activeTabId) => {
          ...
          if (current)
            activity.browser = {
              url: current.url,
              needsReopen: current.needsReopen,
              tabs,
          ...
          void store.save().catch(() => {
            if (activity.browser)
              activity.browser.saveError = 'These tabs could not be saved for next time.';
            publish();
          });
        },
      },
    ...
    const files = new WorkFiles(
      join(
        process.env.DEXTANA_USER_DATA ? app.getPath('userData') : app.getPath('documents'),
        process.env.DEXTANA_USER_DATA ? 'artifacts' : 'Dextana',
      ),
    );
    const showDesktop = (resourceId?: string) => {
      ...
      if (resourceId)
        void notifications
          .command({ action: 'readTarget', target: { kind: 'desktop', resourceId } })
          .catch(console.error);
    };
    ...
    const desktopFailure = (failure: unknown) => {
      ...
      void notifications
        .send({
          source: 'Desktop',
          title: 'Desktop service needs attention',
          body: store.state.desktopError,
          severity: 'error',
          key: store.state.desktopError,
          target: { kind: 'desktop' },
        })
        .catch(console.error);
    };
    const notifications = new Notifications({
      ...
      open: async (item) => {
        ...
        if (target?.kind === 'activity') {
          if (!store.state.activities.some((activity) => activity.id === target.activityId))
    ...
    const computer = new DesktopComputer(
      ...
      new DextComputerCursor(),
      () => {
        if (!window || window.isDestroyed()) return;
        if (window.isMinimized()) window.restore();
        window.show();
        window.focus();
        window.webContents.send('settings:open', 'computer');
      },
    );
    // Computer authority is ephemeral; never restore a saved session after restart.
    ...
    const teachingRequest = async (path: string, method = 'GET', data?: unknown) => {
      await runtime.ensure();
      const response = await runtime.request('/dextana/activity' + path, {
        method,
      ...
      if (!response.ok)
        throw new Error(
          typeof body.detail === 'string'
            ? body.detail
            : 'Teach Dex could not complete that request.',
        );
      return body;
    };
    teachDex = new TeachDex(
      store.state.teaching,
      () =>
        process.env.DEXTANA_TEACH_CAPTURE_FIXTURE
          ? new FixtureTeachingCapture(process.env.DEXTANA_TEACH_CAPTURE_FIXTURE)
          : new CuaTeachingCapture(),
      teachingRequest,
      (snapshot) => {
        store.state.teaching = snapshot;
        publish();
        void store.save().catch(desktopFailure);
      },
    );
    store.state.teaching = teachDex.snapshot();
    ...
    mcp.onCatalogChanged = () => activities.refreshMCPCatalog();
    ...
    app.on('before-quit', (event) => {
      ...
      const teachingStopped = teachDex?.dispose();
      ...
      void Promise.allSettled([
        ...
        timersStopped,
        workflowsStopped,
        fusedCLIInstall.stop(),
        schedulerStopped,
        fusedCLI.stop().then(() => mcp.close()),
        browsers.flush(),
      ])
        .then(() => notifications.flush())
        .then(() => Promise.allSettled([store.flush()]))
        .finally(() => {
          browsers.close();
          app.quit();
        });
    ...
    handle('teach:skills', () => teachDex!.list());
    handle('teach:command', async (command: TeachCommand) => {
      if (!command || typeof command !== 'object') throw new Error('Invalid Teach Dex command.');
      if (command.action === 'pause_run') {
        const run = teachDex!.snapshot().run;
        const result = await teachDex!.command(command);
        if (run) await activities.cancel(run.activityId);
        return result;
      }
      if (command.action === 'resume_run') {
        const run = teachDex!.snapshot().run;
        if (!run) throw new Error('No supervised skill is running.');
        await activities.resume(run.activityId);
      }
      if (command.action === 'stop_run') {
        const run = teachDex!.snapshot().run;
        if (run) await activities.cancel(run.activityId);
        return teachDex!.command(command);
      }
      return teachDex!.command(command);
    });
    handle('desktop:computer', async (request: ComputerCommand) => {
      switch (request.action) {
        case 'enable':
          return computer.enable();
        case 'permissions':
          return computer.enable(true);
        case 'open-accessibility-settings':
          return computer.openSettings('accessibility');
        case 'open-screen-recording-settings':
          return computer.openSettings('screenRecording');
        case 'stop':
          computer.stop();
          break;
        case 'disable':
          ...
          break;
        default:
          throw new Error('Unsupported computer request.');
      }
    ...
    handle('desktop:context', async (activityId, itemId) => {
      ...
      if (item.desktop.operation === 'browser.attach') {
    ...
    handle('files:pick', async () => {
      const result = await dialog.showOpenDialog(window, {
        ...
        filters: [
          { name: 'Documents, spreadsheets and images', extensions: documentExtensions },
          { name: 'Documents', extensions: ['docx', 'pdf', 'txt', 'md'] },
    ...
    handle('files:reveal', (activityId, itemId) => {
      const item = store.state.activities
        .find((a) => a.id === activityId)
        ?.context?.find((item) => item.id === itemId && item.kind === 'file');
      if (!item) throw new Error('This file is not in the chat context.');
    ...
    handle('mcp:auth:open', async (id, surface) => {
      if (!['in-app', 'user'].includes(surface))
        throw new Error('Choose Dextana or your default browser.');
      const authentication = mcp.authenticationURL(id);
      if (surface === 'user') {
        await shell.openExternal(authentication.url);
        return;
      }
      if (
        !store.state.activities.some((activity) => activity.id === authentication.prompt.activityId)
      )
        throw new Error('The chat that requested authentication is no longer available.');
      showActivity(authentication.prompt.activityId);
      browsers.select(authentication.prompt.activityId);
      await browsers.newTab(authentication.prompt.activityId, authentication.url, false, true);
    });
    handle('mcp:auth:dismiss', (id) => mcp.dismissAuthentication(id));
    const settingsRequest = async (path: string, method = 'GET', data?: unknown) => {
      ...
      if (!response.ok)
        throw new Error(
          typeof body.detail === 'string' ? body.detail : 'Could not update settings.',
        );
      return body;
    };
    ...
    handle('skills:import', async () => {
      const result = await dialog.showOpenDialog(window, {
        ...
        properties: ['openFile'],
        filters: [{ name: 'Skill document', extensions: ['md'] }],
      });
    ...
    handle('activity:model', (id, model, reasoning) =>
      activities.selectModel(id, model, reasoning),
    );
    ...
    handle('browser:user:extension', async () => {
      const folder = await prepareBrowserExtension({
    ...
    handle('browser:show', (id, tabId) => {
      if (typeof id !== 'string' || !store.state.activities.some((activity) => activity.id === id))
    ...
    handle('browser:login:begin', async (id) => {
      ...
      const destination =
        store.state.activities.find((a) => a.id === target.activityId)?.title ?? 'Dextana';
    ...
    handle('browser:login:extension', async (id) => {
      ...
      try {
        ...
        throw new Error(
          'Could not open the bundled extension. Please reinstall or rebuild Dextana.',
        );
      }
    });
    handle('browser:login:status', (id) => loginTransfer.status(id));
    handle('browser:login:cancel', (id) => {
      if (loginTransfer.cancel(id)) browsers.setTransferOverlay(false);
    });
    handle('browser:hide', () => browsers.hide());
    handle('browser:overlay', (visible) => {
      if (typeof visible !== 'boolean') throw new Error('Invalid overlay state');
      browsers.setAppOverlay(visible);
    });
    ...
    handle('fused:cli:choose', async () => {
      const selected = await dialog.showOpenDialog(window, {
        title: 'Locate Fused CLI',
        properties: ['openFile', 'showHiddenFiles'],
      });
      return selected.canceled || !selected.filePaths[0]
        ? fusedCLIInstall.status()
        : fusedCLIInstall.useFile(selected.filePaths[0]);
    });
    ...
    handle('fused:operations', (id) => fusedCLI.operations(id));
    handle('fused:discover', () => fusedCLI.discover());
    handle('fused:logout', async () => {
      await mcp.revokeFused();
      await fusedCLI.logout();
    });
    handle('fused:select', (id, autoToken, operations) =>
      mcp.selectFused(id, autoToken, operations),
    );
    handle('fused:account:save', async (input) => {
      await fused.saveAccount(input);
      publish();
    });
    handle('fused:account:remove', async () => {
      await fused.removeAccount();
      publish();
    });
    handle('fused:remove', async (id) => {
      await fused.remove(id);
      publish();
    });
    ...
    handle('activity:session-approvals', (id, allowAll) =>
      activities.setSessionApprovals(id, allowAll),
    );
    ...
    powerMonitor.on('resume', () => {
      void timeFiles.tick().catch(desktopFailure);
    });
    const createWindow = () => {
      ...
      const stopComputerUse = () => {
        if (computer.snapshot().active?.state === 'stopped') return;
        try {
          computer.stop();
        } catch (error) {
          console.error(error);
      ...
      window.on('close', (event) => {
        stopComputerUse();
        if (!quitting && tray && store.state.desktopBackground !== false) {
          event.preventDefault();
          window.hide();
        }
      });
      window.on('hide', stopComputerUse);
      window.on('closed', stopComputerUse);
      window.on('focus', () => {
        void notifications.refreshVisibility().catch(console.error);
      });
    ...
    installApplicationMenu(
      ...
      () => {
        ...
        const item = Menu.getApplicationMenu()?.getMenuItemById('restart-agent');
        if (item) item.enabled = false;
        void activities
          .restartBackend(buildDevelopmentBackend)
          .then(() =>
            dialog.showMessageBox(window, {
              type: 'info',
              message: 'Agent rebuilt and restarted.',
              detail: 'Your chats are ready. The app window stayed open.',
            }),
          )
          .catch((error) => dialog.showErrorBox('Agent restart failed', error.message))
          .finally(() => {
