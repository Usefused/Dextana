...
export class LocalCapabilities {
    ...
    if (desktop)
      this.desktop = new DesktopInvocations(
        ...
        (activity, work) =>
          ...
          (activity.allowAllApprovals === true ||
            activity.permissions?.[work === 'computer' ? 'computer' : 'desktop'] === true),
        this.receipt,
      );
  }
  ...
  async setPermission(input: Parameters<DesktopAPI['setPermission']>[0]) {
    if (
      !input ||
      !['browser', 'mcp', 'fileRead', 'fileCreate', 'desktop', 'computer'].includes(
        input.capability,
      ) ||
  ...
  private validateAutoAllow(activity: Activity, input: Parameters<DesktopAPI['approve']>[0]) {
    ...
    if (
      activity.approval?.source === 'harnest' &&
      !['mcp', 'computer'].includes(activity.approval.capability)
    )
  ...
  async decideRuntimeApproval(activity: Activity, event: any, signal: AbortSignal) {
    if (event.approval?.action === 'dynamic:desktop.execute') {
      ...
      return this.approval(
        activity,
        plan.operation.work === 'computer' ? 'computer' : 'desktop',
