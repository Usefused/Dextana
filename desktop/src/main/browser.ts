...
async function pageScript(contents: Electron.WebContents, scripts: Electron.WebSource[]) {
  ...
  const result = await contents
    ...
    .catch(() => {
      throw new Error(
        'The browser could not run its page inspection. The last action is unverified. Read the page before retrying; this does not establish that the website blocked the action.',
      );
    });
  if (!result?.ok)
    throw new Error(
      result?.error || 'Could not inspect this page. Read it again before taking another action.',
    );
  return result.value;
}
const overlaySelector =
  'dialog[open],[role="dialog"],[role="alertdialog"],[aria-modal="true"],[popover]:popover-open,[role="menu"],[role="listbox"]';
const controlSelector =
  'a,button,input,textarea,select,[role="button"],[role="link"],[contenteditable="true"],[role="textbox"],[role="searchbox"],[role="combobox"],[role="menuitem"],[role="option"]';
const readPage = `(() => {
...
const cursor = `(() => {
  ...
  if (!host) {
    ...
    host.style.cssText='position:fixed;left:24px;top:30px;width:${agentCursorWidth}px;height:${agentCursorHeight}px;z-index:2147483647;pointer-events:none;transform-origin:center;transition:left 260ms cubic-bezier(.22,.8,.3,1),top 260ms cubic-bezier(.22,.8,.3,1)';
    const shadow = host.attachShadow({mode:'closed'});
    shadow.innerHTML=${JSON.stringify(agentCursorMarkup)};
    document.documentElement.appendChild(host);
  }
  globalThis.__dextanaCursor = host;
  globalThis.__dextanaMoveCursor = (x,y) => {
    host.style.left=(x-${agentCursorHotspot.x})+'px';
    host.style.top=(y-${agentCursorHotspot.y})+'px';
    globalThis.__dextanaCursorMotion?.cancel();
    if (!matchMedia('(prefers-reduced-motion: reduce)').matches) {
      globalThis.__dextanaCursorMotion=host.animate(
        [{transform:'translateY(1px) scale(.88)',opacity:.72},{transform:'translateY(0) scale(1)',opacity:1}],
...
export class Browsers {
  ...
  private loginTimer = setInterval(() => {
  ...
  /** Owner-visible tabs whose sensitive URLs must not be saved or exposed to the agent. */
  ...
  private async checkLogin() {
    ...
    try {
      ...
      if (this.dismissedLogins.get(id) === url) {
        this.loginOffer.close();
        return;
      }
      this.loginOffer.show(
        this.host(),
        view.getBounds(),
        (action) => {
          this.dismissedLogins.set(id, url);
          this.loginOffer.close();
          if (action === 'transfer' && this.activeId === id && view.webContents.getURL() === url)
            this.offerLogin?.(id);
        },
        `${id}:${url}`,
      );
  ...
  private positionLogin = () => {
    if (this.active) this.loginOffer.position(this.host(), this.active.getBounds());
  };
  ...
  setTransferOverlay(visible: boolean) {
    ...
    if (visible) {
      ...
      if (this.activeId && this.active)
        this.dismissedLogins.set(this.activeId, this.active.webContents.getURL());
    }
  ...
  private persist(activityId: string) {
    const tabs = this.owned(activityId).filter((tab) => !this.ephemeral.has(tab.id));
    const defaultTabId = this.defaults.get(activityId);
    this.bookmarks.save(
      activityId,
      tabs.map((tab) => ({ ...tab })),
      tabs.some((tab) => tab.id === defaultTabId) ? defaultTabId : undefined,
    );
    this.notify();
  }
  private notify() {
    ...
    this.changed(
      tab
        ? {
            ephemeral: this.ephemeral.has(tab.id) || undefined,
  ...
  private create(activityId: string, url: string, ephemeral = false) {
    if (this.userBrowser?.has(activityId))
      throw new Error('The browser selection changed. Request the action again.');
    ...
    if (ephemeral) this.ephemeral.add(tab.id);
  ...
  setPaneWidth(width: unknown, dragging: unknown) {
    if (
      (width !== undefined &&
        (typeof width !== 'number' || !Number.isFinite(width) || width < 320 || width > 16384)) ||
  ...
  private get(id: string) {
    ...
    if (!window || window.webContents.isDestroyed()) {
      ...
      window.setBounds({
        x: hostWidth - paneWidth,
        y: 164,
        width: paneWidth,
        height: Math.max(100, hostHeight - 164),
      });
      ...
      if (!this.configuredSessions.has(session)) {
        ...
        session.on('will-download', (event, item, contents) => {
          ...
          this.downloads.start(
            ...
            tab.activityId,
            tab.id,
            this.planOrigins.get(source.id)?.origins,
          );
        });
        session.webRequest.onBeforeRequest((details, callback) => {
          const scope =
            details.webContentsId === undefined
              ? undefined
              : this.planOrigins.get(details.webContentsId);
          const outsidePlan =
            scope &&
            details.resourceType === 'mainFrame' &&
            !scope.origins.includes(new URL(details.url).origin);
          if (outsidePlan) scope.blocked = details.url;
          callback({
            cancel:
              !!outsidePlan ||
              !browserResourceAllowed(details.url, details.frame?.url ?? '', details.resourceType),
          });
        });
      }
  ...
  private async loadPage(
    ...
    const previous = new Set(
      this.downloads.desktopRecords(tab.activityId, tab.id).map((item) => item.id),
    );
    try {
      await contents.loadURL(url);
    } catch (error) {
      // Chromium aborts navigation when an attachment becomes a download.
      if (!['ERR_ABORTED', 'ERR_FAILED'].includes((error as { code?: string }).code ?? ''))
        throw error;
      await delay(100, undefined, { signal });
      const started = this.downloads
        .desktopRecords(tab.activityId, tab.id)
        .filter((item) => !previous.has(item.id));
      if (!started.length) throw error;
      return started.map(downloadReceipt);
    }
  }
  private async loadSaved(tab: BrowserTab, inspect = true) {
    ...
    const opening = (async () => {
      try {
        ...
        if (downloads) {
          this.remember(tab.id, url);
          return;
        }
        // Owner-only authentication pages must load without Dextana inspecting their contents.
        if (inspect) await pageScript(view.webContents, [{ code: cursor }]);
  ...
  async newTab(activityId: string, url: string, inspect = true, ephemeral = false) {
    const previousDefault = this.defaults.get(activityId);
    const tab = this.create(activityId, browserURL(url), ephemeral);
    if (previousDefault) {
      this.defaults.set(activityId, previousDefault);
      this.persist(activityId);
    }
    if (inspect) await this.reopen(activityId, tab.id);
    else {
      this.viewed.set(activityId, tab.id);
      await this.loadSaved(tab, false);
      this.show(tab.id);
    }
  }
  async refresh(id: string) {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        const closed = () => {
          cleanup();
          reject(new Error('The browser tab was closed.'));
        };
        const failed = (
          _event: unknown,
          code: number,
          ...
          cleanup();
          reject(new Error(`Could not refresh this page: ${description}`));
        };
        const timer = setTimeout(() => {
          cleanup();
          reject(new Error('The page took too long to refresh.'));
        }, 30_000);
  ...
  downloadAction(id: unknown, action: unknown) {
    if (typeof id !== 'string' || !['cancel', 'reveal'].includes(String(action)))
      throw new Error('Invalid download action.');
    if (action === 'cancel') this.downloads.cancel(id);
    else this.downloads.reveal(id);
  }
  releaseUserBrowser(activityId: string) {
    this.userBrowser?.release(activityId);
  }
  ...
  private prepareInApp(activityId: string, args: Record<string, unknown>) {
    ...
    const id =
      requested && !this.ephemeral.has(requested)
        ? requested
        : this.owned(activityId).find((tab) => !this.ephemeral.has(tab.id))?.id;
  ...
  async execute(
    ...
    if (args.action === 'list_tabs')
      return {
        tabs: this.owned(activityId)
          .filter((tab) => !this.ephemeral.has(tab.id))
          .map((tab) => ({
            tab_id: tab.id,
            title: tab.title,
            url: tab.url,
    ...
    if (tab && this.ephemeral.has(tab.id)) tab = undefined;
    ...
    // Saved tabs are part of the browser workflow. Restore the page before carrying
    // out the newly requested action instead of asking the owner to reopen it first.
    // Only the saved navigation is restored; no earlier interaction is replayed.
    ...
    if (
      screenshotPoint &&
      (!['click', 'hover', 'scroll'].includes(String(args.action)) || args.ref)
    )
      throw new Error(
        'Screenshot coordinates require click, hover, or scroll without an element reference.',
      );
    if (!screenshotPoint && args.screenshot_id)
      throw new Error('Choose screenshot or normalized coordinate_space with screenshot_id.');
    if (!screenshotPoint && !['read', 'screenshot'].includes(String(args.action)))
      this.screenshots.invalidate(id);
    this.defaults.set(activityId, id);
    this.persist(activityId);
    const window = this.get(id);
    const scope = approvedOrigins
      ? { origins: approvedOrigins, blocked: undefined as string | undefined }
      : undefined;
    ...
    try {
      ...
      if (args.action === 'open' || args.action === 'new_tab') {
        const downloads = await this.loadPage(
          tab,
          window.webContents,
          browserURL(args.url),
          signal,
        ...
        if (downloads)
          return {
            ...
            downloads,
            message:
              'Navigation started a download. Only a completed download with a saved path proves the file was saved.',
          };
      }
      let inspection = this.inspections.get(window.webContents);
      if (!inspection) {
        inspection = new BrowserInspection(window.webContents);
        this.inspections.set(window.webContents, inspection);
      }
      ...
      if (['open', 'new_tab', 'read'].includes(String(args.action)) && !args.ref)
        return {
          ...(await inspection.snapshot(readPage, args)),
          tab_id: id,
          downloads: this.downloads.receipts(activityId, id),
        };
      if (args.action === 'screenshot') {
        ...
        return {
          tab_id: id,
          image: { type: 'image', mediaType: 'image/png', data: png.toString('base64') },
          ...this.screenshots.capture(id, png, before, after),
        };
      }
      ...
      const frame = inspection.frame(
        ...
        args.action === 'press',
        ['click', 'fill', 'press'].includes(String(args.action)),
      );
      ...
      if (args.action === 'read' && args.ref) {
        ...
        return {
          ...detail,
          text: detail.text.slice(offset, offset + 24000),
          text_total: detail.text.length,
          next_text_offset: offset + 24000 < detail.text.length ? offset + 24000 : null,
          ref: args.ref,
          tab_id: id,
        };
      }
      ...
      if (coordinate || args.action === 'scroll') {
        ...
        const location = screenshotPoint
          ? this.screenshots.point(
              id,
              args,
              await pageScript(window.webContents, [{ code: screenshotGeometry }]),
            )
          : args;
        const x = location.x,
          y = location.y;
        if (
          ...
          throw new Error(
            'Choose coordinates inside the browser viewport from the latest screenshot or bounds.',
          );
        const zoom = window.webContents.getZoomFactor();
        const point = { x: Math.round(x * zoom), y: Math.round(y * zoom) };
        await run([{ code: `globalThis.__dextanaMoveCursor(${x},${y})` }]);
      ...
      if (
        !coordinate &&
        (args.action === 'click' || args.action === 'fill' || args.action === 'hover')
      ) {
        ...
        await run([
          {
            code: `(() => { const a=${action}; const el=globalThis.__dextanaElements?.[a.index]; if(!el || !el.isConnected) throw new Error('Element is stale. Read the page again.'); el.scrollIntoView({block:'center',inline:'nearest'}); const r=el.getBoundingClientRect(); globalThis.__dextanaMoveCursor(r.left + Math.min(r.width/2,80),r.top+r.height/2); })()`,
          },
        ]);
        await delay(280, undefined, { signal });
        signal.throwIfAborted();
      ...
      if (args.action === 'press') {
        ...
        if (hasRef && (typeof args.ref !== 'string' || !/^(?:f\d+:)?[1-9]\d*$/.test(args.ref)))
          throw new Error(
            'Use an element reference from the last browser read, or omit ref to press a key at the current focus.',
          );
        ...
        if (key === 'Copy') this.clipboards.set(activityId, selected);
          ...
          if (text === undefined)
            throw new Error('Copy text in this activity first. The system clipboard is not used.');
      ...
      signal.throwIfAborted();
      return {
        ...(await inspection.snapshot(readPage, {})),
        tab_id: id,
        downloads: this.downloads.receipts(activityId, id),
      };
    } catch (error) {
      if (scope?.blocked)
        throw new Error(
          `This page redirected outside the approved plan to ${scope.blocked}. Request permission to open that address.`,
        );
      throw error;
    } finally {
