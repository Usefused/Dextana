...
import { fusedAuthentication, openFusedMCP } from './fused-mcp-client';
...
function initialFusedToolPolicy(name: string): MCPToolPolicy {
  if (name === 'search_docs') return 'auto';
  if (name === 'execute') return 'ask';
  return 'disabled';
}
...
export class MCPConnections {
  ...
  private nativeScope(connection: MCPConnection) {
    return JSON.stringify([connection.fusedNative, this.store.state.fusedWorkspace?.connectedAt]);
  }
  private lease(connection: MCPConnection, activityId: string) {
    const lease = this.tokens.get(`${connection.id}:${activityId}`);
    return lease &&
      lease.value.expiresAt > Date.now() + 1000 &&
      lease.scope === this.nativeScope(connection)
      ? lease.value
      : undefined;
  }
  private clients = new Map<string, Promise<Client>>();
  private authenticationURLs = new Map<string, string>();
  private authenticationTimers = new Map<string, ReturnType<typeof setTimeout>>();
  private authenticationWaiters = new Map<string, AuthenticationWaiter>();
  ...
  constructor(
    ...
    private openNative: typeof openFusedMCP = openFusedMCP,
  ) {}
  ...
  async selectFused(id: string, autoToken: boolean, operations: string[]) {
    ...
    if (!workspace || !server)
      throw new Error('Refresh Fused and select a discovered MCP version.');
    if (
      typeof autoToken !== 'boolean' ||
      !Array.isArray(operations) ||
      operations.length > 100 ||
      operations.some(
        (value) =>
          typeof value !== 'string' || !value.trim() || value.length > 300 || value.includes('*'),
      )
    )
      throw new Error('Choose exact operation IDs, without wildcards.');
    const previous = this.store.state.mcpConnections?.find(
      (item) => item.fusedNative?.engine === workspace.url && item.fusedNative.server.id === id,
    );
    const connectionId = previous?.id ?? randomUUID();
    await this.exclusive(connectionId, async () => {
      const connection: MCPConnection = {
        id: connectionId,
        name: `${server.name} · ${server.version}`,
        transport: 'http',
        url: server.url,
        command: '',
        args: [],
        ...
        fusedNative: {
          engine: workspace.url,
          server,
          autoToken,
          operations: [...new Set(operations.map((value) => value.trim()))],
        },
      };
    ...
    return this.exclusive(id, async () => {
      ...
      const auth =
        input.transport === 'http'
          ? authFrom(
              input.auth ??
                previous?.auth ?? { type: input.token || previous?.secretId ? 'bearer' : 'none' },
            )
          : undefined;
      if (auth && auth.type !== 'body' && input.token && /[\x00-\x1f\x7f]/.test(input.token))
        throw new Error('Header tokens cannot contain line breaks or control characters.');
      const sameEndpoint =
          ...
          JSON.stringify(
            previous?.auth ??
              (previous?.transport === 'http'
                ? { type: previous.secretId ? 'bearer' : 'none' }
                : undefined),
          );
      const customAuth =
        auth?.type === 'custom' && input.customAuth !== undefined
          ? customMCPAuth(input.customAuth)
          : undefined;
      const clearSecret = auth?.type === 'none';
      const replacingSecret =
        !clearSecret &&
        (input.token !== undefined || input.environment !== undefined || customAuth !== undefined);
      let secretId = clearSecret ? undefined : sameTarget ? previous?.secretId : undefined;
      if (auth?.type === 'custom' && !customAuth && !secretId)
        throw new Error('Enter custom authentication for this connection.');
      if (
        auth &&
        !['none', 'custom'].includes(auth.type) &&
        !(input.token || (input.token === undefined && secretId))
      )
        throw new Error('Enter an auth token for this connection.');
      if (replacingSecret) {
        if (
        ...
        await writeFile(
          ...
          safeStorage.encryptString(
            JSON.stringify({
              token: auth?.type === 'custom' ? '' : (input.token ?? ''),
  ...
  private async open(connection: MCPConnection, signal: AbortSignal, nativeToken?: string) {
    if (connection.fusedNative && !nativeToken)
      throw new Error('This Fused MCP requires a scoped connection token.');
    signal.throwIfAborted();
    const secret = nativeToken
      ? { token: nativeToken, environment: {} }
      : connection.secretId
        ? JSON.parse(
        ...
        : { token: '', environment: {} };
    if (connection.fusedNative) {
      const userRef = this.store.state.dextIdentityRef;
      if (!userRef) throw new Error('Dext identity is unavailable. Reconnect the Fused workspace.');
      return this.openNative({
        url: connection.url,
        token: nativeToken!,
        userRef,
        signal,
        onAuthenticationComplete: (id) => this.authenticationComplete(id),
      });
    }
    ...
    return backendMCP(
      ...
      connection.transport === 'http'
        ? {
            ...
            url: connection.url,
            token: secret.token,
            customAuth: secret.customAuth,
            auth: nativeToken ? { type: 'bearer' } : connection.auth,
          }
        : {
            transport: 'stdio',
  ...
  private requestAuthentication(
    activityId: string,
    connection: MCPConnection,
    authentication: NonNullable<ReturnType<typeof fusedAuthentication>>,
    signal: AbortSignal,
  ) {
    signal.throwIfAborted();
    if (this.authenticationWaiters.has(authentication.id))
      throw new Error('This provider authentication request is already pending.');
    const prompts = (this.store.state.mcpAuthentications ?? []).filter(
      (item) => item.id !== authentication.id && item.state !== 'expired',
    );
    this.authenticationURLs.set(authentication.id, authentication.url);
    clearTimeout(this.authenticationTimers.get(authentication.id));
    const timer = setTimeout(
      () => {
        this.authenticationURLs.delete(authentication.id);
        this.authenticationTimers.delete(authentication.id);
        const prompt = this.store.state.mcpAuthentications?.find(
          (item) => item.id === authentication.id,
        );
        ...
        this.settleAuthentication(
          authentication.id,
          new Error('Provider authentication expired. The provider action was not run.'),
        );
      },
      Math.max(1, Date.parse(authentication.expiresAt) - Date.now()),
    );
    timer.unref();
    this.authenticationTimers.set(authentication.id, timer);
    this.store.state.mcpAuthentications = [
      ...prompts.slice(-4),
      {
        ...
        state: 'waiting',
        retryAllowed: authentication.retryAllowed,
        expiresAt: authentication.expiresAt,
      },
    ];
    this.publish();
    if (!authentication.retryAllowed) return;
    return new Promise<void>((resolve, reject) => {
      const abort = () => {
        this.removeAuthentication(authentication.id);
        this.settleAuthentication(
          authentication.id,
          new Error('Provider authentication was cancelled. The provider action was not run.'),
        );
      };
      this.authenticationWaiters.set(authentication.id, {
        activityId,
        signal,
        abort,
        resolve,
  ...
  private settleAuthentication(id: string, error?: Error) {
    const waiter = this.authenticationWaiters.get(id);
    if (!waiter) return;
    this.authenticationWaiters.delete(id);
    waiter.signal.removeEventListener('abort', waiter.abort);
    if (error) waiter.reject(error);
    else waiter.resolve();
  }

  private removeAuthentication(id: string) {
    this.store.state.mcpAuthentications = this.store.state.mcpAuthentications?.filter(
      (item) => item.id !== id,
    );
    this.authenticationURLs.delete(id);
    clearTimeout(this.authenticationTimers.get(id));
    this.authenticationTimers.delete(id);
    this.publish();
  }

  private authenticationComplete(id: string) {
    const prompt = this.store.state.mcpAuthentications?.find((item) => item.id === id);
    if (!prompt || prompt.state !== 'waiting') return;
    if (prompt.retryAllowed) {
      this.removeAuthentication(id);
      this.settleAuthentication(id);
      return;
    }
    prompt.state = 'complete';
    this.authenticationURLs.delete(id);
    clearTimeout(this.authenticationTimers.get(id));
    this.authenticationTimers.delete(id);
    this.publish();
  ...
  authenticationURL(id: unknown) {
    if (typeof id !== 'string') throw new Error('Invalid authentication request.');
    const prompt = this.store.state.mcpAuthentications?.find((item) => item.id === id);
    const url = this.authenticationURLs.get(id);
    if (!prompt || prompt.state !== 'waiting' || !url)
      throw new Error('This authentication link is no longer available.');
    if (Date.parse(prompt.expiresAt) <= Date.now())
      throw new Error('This authentication link has expired. Request the action again.');
    return { prompt, url };
  }

  dismissAuthentication(id: unknown) {
    if (typeof id !== 'string') throw new Error('Invalid authentication request.');
    if (!this.store.state.mcpAuthentications?.some((item) => item.id === id)) return;
    this.removeAuthentication(id);
    this.settleAuthentication(
      id,
      new Error('Provider authentication was cancelled. The provider action was not run.'),
  ...
  private async testConnection(connection: MCPConnection): Promise<MCPConnection> {
    ...
    const valid = () => {
      ...
      if (
        native &&
        (scope !== this.nativeScope(connection) ||
          this.store.state.fusedWorkspace?.url !== native.engine)
      )
        throw new Error('Fused workspace changed. Test the connection again.');
    };
    let temporaryToken: FusedToken | undefined;
    let client: Client | undefined;
    try {
      if (native) {
        if (this.store.state.fusedWorkspace?.url !== native.engine)
          throw new Error(
            'Reconnect your Fused workspace in Settings before testing this connection.',
          );
        if (!this.fusedCLI)
          throw new Error('Fused CLI token service is unavailable. No token was created.');
        // An owner-initiated settings test authorizes discovery, never an agent lease.
        temporaryToken = await this.fusedCLI.issue(
          native.engine,
          native.server.mcpId,
          native.operations,
          signal,
      ...
      const tools = discovered.map((tool) => {
        ...
        return {
          ...
          policy:
            previous?.policy ??
            (connection.fusedNative ? initialFusedToolPolicy(tool.name) : 'disabled'),
        } as MCPTool;
      });
      ...
      try {
        ...
        if (temporaryToken)
          await this.fusedCLI!.revoke(temporaryToken).catch(() => {
            throw new Error(
              'Could not confirm cleanup of the temporary test token. It expires within 24 hours.',
            );
          });
      }
    }
      ...
      .filter(
        (connection) =>
          ...
          (!connection.fusedNative ||
            (connection.fusedNative.autoToken &&
              this.store.state.fusedWorkspace?.url === connection.fusedNative.engine)),
      )
      .map((connection) => ({
        id: connection.id,
        ...
        tools: connection.tools
          .filter(
            (tool) => tool.policy !== 'disabled' && tool.fingerprint !== 'fused-native-connect-v1',
          )
          .map(({ name, description, inputSchema }) => ({ name, description, inputSchema })),
      }))
      .filter((connection) => connection.tools.length);
  }
  private sessionAllows(activityId: string) {
    const activity = this.store.state.activities.find((item) => item.id === activityId);
    return activity?.allowAllApprovals === true || activity?.permissions?.mcp === true;
  }
  prepare(
    activityId: string,
    callId: string,
    ...
    if (
      ...
      throw new Error(
        'Enable automatic tokens for this MCP in your connected Fused workspace first.',
      );
    const needsToken = !!connection.fusedNative && !this.lease(connection, activityId);
    const tool = connection.tools.find(
      (item) => item.name === toolName && item.fingerprint !== 'fused-native-connect-v1',
    );
    ...
    const sessionAllows = this.sessionAllows(activityId);
    ...
    return {
      ...
      message: needsToken
        ? `${connection.name} · Create agent token and ${tool.name}`
      ...
      arguments: {
        ...
        toolName,
        args,
        ...(needsToken && connection.fusedNative
          ? {
              token: {
                mcpId: connection.fusedNative.server.mcpId,
                version: connection.fusedNative.server.version,
                endpoint: connection.url,
                operations: connection.fusedNative.operations.length
                  ? connection.fusedNative.operations
                  : ['*'],
                expiresIn: FUSED_TOKEN_LIFETIME,
                scope:
                  'Token applies to these operations across MCP versions; Dext pins this endpoint.',
                purpose: `Create a scoped token, then run ${tool.name} with these arguments.`,
              },
            }
          : {}),
      },
  ...
  async execute(activityId: string, callId: string, ticket: string, signal: AbortSignal) {
    ...
    valid();
    if (plan.sessionAllowed && !this.sessionAllows(activityId))
      throw new Error('Session approval setting changed. Request the action again.');
    if (plan.connection.fusedNative) {
      if (this.store.state.fusedWorkspace?.url !== plan.connection.fusedNative.engine)
        throw new Error('Fused workspace disconnected. Connect again in Settings.');
      if (plan.needsToken && !this.lease(plan.connection, activityId)) {
        ...
        const issued = await this.fusedCLI.issue(
          native.engine,
          native.server.mcpId,
          native.operations,
          signal,
        ...
        try {
          valid();
          if (scope !== this.nativeScope(this.find(plan.connection.id)))
            throw new Error('Fused account changed.');
          client = await this.open(plan.connection, signal, issued.token);
          const tools = (await this.discover(client, signal)).map((tool) => ({
            ...tool,
            description: tool.description ?? '',
            fingerprint: toolFingerprint(tool),
            policy:
              plan.connection.tools.find(
                (previous) =>
                  previous.name === tool.name && previous.fingerprint === toolFingerprint(tool),
          ...
          if (!live || live.fingerprint !== plan.tool.fingerprint)
            throw new Error('The tool changed. Review its permissions again.');
          ...
          const timer = setTimeout(
            () => {
              ...
              this.expiryTimers.delete(tokenKey);
              const pending = this.clients.get(tokenKey);
          ...
          timer.unref();
          this.expiryTimers.set(tokenKey, timer);
          if (previous) void this.fusedCLI.revoke(previous.value).catch(() => {});
        } catch {
          await this.fusedCLI.revoke(issued).catch(() => {});
          throw new Error(
            'Fused connection setup failed. Token revocation was attempted; no service action was executed.',
          );
        } finally {
          await client?.close().catch(() => {});
        }
      }
      if (!this.lease(plan.connection, activityId))
        throw new Error('Fused token expired. Request the action again to approve a new token.');
    }
    ...
    valid();
    if (plan.connection.fusedNative && !this.lease(plan.connection, activityId))
      throw new Error('Fused token expired. Request the action again to approve a new token.');
    let authenticationRetries = 0;
    while (true) {
      let result;
      try {
        result = await client.callTool({ name: plan.tool.name, arguments: plan.args }, undefined, {
          signal,
          timeout: 60000,
        });
      } catch (error) {
        const authentication = plan.connection.fusedNative ? fusedAuthentication(error) : undefined;
        if (!authentication)
          throw new Error(
            'MCP execution did not return a usable result. Its outcome may be unknown; do not automatically retry.',
          );
        if (!authentication.retryAllowed) {
          this.requestAuthentication(activityId, plan.connection, authentication, signal);
          throw new Error(
            'Provider authentication is required. Choose where to open the connection link in Dextana. The previous operation has an unknown outcome and must not be replayed automatically.',
          );
        }
        if (authenticationRetries >= 5)
          throw new Error(
            'The provider requested authentication too many times. No provider action was run.',
          );
        authenticationRetries += 1;
        await this.requestAuthentication(activityId, plan.connection, authentication, signal);
        valid();
        if (plan.connection.fusedNative && !this.lease(plan.connection, activityId))
          throw new Error('Fused token expired while authenticating. Request the action again.');
        continue;
      }
  ...
  release(activityId: string) {
    ...
    for (const [id, waiter] of this.authenticationWaiters)
      if (waiter.activityId === activityId) waiter.abort();
  }
  ...
  async close() {
    ...
    this.authenticationURLs.clear();
    for (const [id] of this.authenticationWaiters) {
      this.removeAuthentication(id);
      this.settleAuthentication(id, new Error('Dextana closed before authentication completed.'));
    }
    delete this.store.state.mcpAuthentications;
    await Promise.allSettled(clients.map(async (client) => (await client).close()));
  }
}
