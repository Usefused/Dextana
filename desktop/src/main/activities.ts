...
export class Activities {
  ...
  constructor(private store: Store, private runtime: Runtime, private publish: () => void, browsers: Browsers, fused: Fused, private mcp?: MCPConnections, files?: WorkFiles, desktop?: DesktopGateway, private onActivities?: (activities: Activity[]) => void) {
  ...
  initialize() {
    this.ready ??= (async () => {
      ...
      const data = await this.request('/command', { action: 'initialize', mcpCatalog: this.mcpCatalog(), settings: this.store.state.settings, ...(this.store.backendOwnsActivities ? {} : { activities: this.store.state.activities.filter(activity => Array.isArray(activity.messages)) }), local: this.store.state.activities.map(localActivity) });
      this.apply(data);
  ...
  private async poll() {
    ...
    while (!this.stopped && generation === this.pollGeneration) {
      try { const state = await this.request('/events?revision=' + this.revision); if (generation !== this.pollGeneration) break; this.apply(state); failures = 0; }
      catch (error) {
        if (this.stopped || generation !== this.pollGeneration) break;
  ...
  private async command(action: string, input?: unknown) {
    if (this.restarting) throw new Error('Agent is restarting. Try again when it is ready.');
    await this.initialize();
    const state = await this.request('/command', { action, input, mcpCatalog: this.mcpCatalog(), settings: this.store.state.settings, local: this.store.state.activities.map(localActivity) });
    this.apply(state);
    return state.result;
  }
  ...
  private mcpCatalog() {
    return (this.mcp?.catalog() ?? []).map(connection => ({ id: connection.id, tools: connection.tools.map(tool => ({ name: tool.name })) }));
  }
  refreshMCPCatalog() { return this.command('initialize'); }
  ...
  async restartBackend(build: () => Promise<void>) {
    if (this.restarting) throw new Error('Agent is already restarting.');
    if (this.pending.size || this.store.state.activities.some(activity =>
      ['starting', 'running'].includes(activity.status) || activity.approval || activity.queue?.length))
      throw new Error('Finish or stop active work and queued messages before restarting the agent.');
    this.restarting = true;
    this.pollGeneration++;
    this.ready = undefined;
    try {
      await this.runtime.rebuild(build);
    } finally {
      this.revision = -1;
      this.localVersion = '';
      try { await this.initialize(); } finally { this.restarting = false; }
    }
  }
