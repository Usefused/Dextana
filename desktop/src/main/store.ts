...
export class Store {
  ...
  async load() {
    ...
    this.state.browserPreferences = {
      autoAllow: this.state.browserPreferences?.autoAllow === true,
    };
    ...
    this.state.fusedIntegrations ??=
      this.state.fused.url || this.state.fused.hasToken
        ? [
            {
              ...this.state.fused,
              id: 'legacy-fused',
              name: 'Fused',
              revision: 'legacy',
              secretId: this.state.fused.hasToken ? 'legacy' : undefined,
            },
          ]
        : [];
    ...
    for (const activity of this.state.activities) {
      ...
      if (!activity.context) {
        ...
        for (const message of activity.messages)
          if (message.content) rememberReferences(activity, message.content);
      }
      const url =
        externalURL(activity.browser?.url) ??
        (previousBrowser?.activityId === activity.id
          ? externalURL(previousBrowser.url)
          : undefined);
        ...
        return [
          {
            ...
            url: page,
            title: typeof tab.title === 'string' ? tab.title.slice(0, 160) : new URL(page).hostname,
            needsReopen: true,
          },
        ];
      });
      ...
      if (current)
        activity.browser = { url: current.url, needsReopen: true, tabs, activeTabId: current.id };
      ...
      if (
        ...
        activity.plans?.at(-1)?.status === 'proposed'
      )
      ...
      if (!['completed', 'awaiting_plan'].includes(activity.status))
        delete activity.runtimeSessionId;
      delete activity.approval;
      for (const message of activity.messages)
        if (message.thought) delete message.thought.runningSince;
    }
  }
  flush() {
    return this.writing;
  ...
  save() {
    const {
      cronJobs: _serverJobs,
      cronError: _cronError,
      mcpAuthentications: _mcpAuthentications,
      ...desktopState
    } = this.state;
    if (desktopState.browser?.ephemeral) delete desktopState.browser;
    const data = JSON.stringify(
      {
        ...desktopState,
        activities: this.backendOwnsActivities
          ? this.state.activities.map(localActivity)
          : this.state.activities,
      },
      null,
      2,
