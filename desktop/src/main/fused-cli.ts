...
export function parseFusedToken(
  value: unknown,
  mcpId: string,
  name: string,
  operations: string[],
  ...
  if (
    ...
    /[\r\n]/.test(item.token) ||
    !Array.isArray(item.allow) ||
    item.allow.length !== expectedOperations.length ||
    !expectedOperations.every(
      (operation) => item.allow instanceof Array && item.allow.includes(operation),
    ) ||
    !Number.isFinite(expiry) ||
    expiry <= Date.now() ||
    expiry > Date.now() + FUSED_TOKEN_LIFETIME_MS + 30_000
  )
    throw new Error('Invalid Fused token response.');
...
export function parseFusedOperations(value: unknown, server: FusedServer): string[] {
  const data = value as {
    version_id?: string;
    mcp_id?: string;
    total?: number;
    operations?: { operation_id?: unknown }[];
  };
  if (
    data?.version_id !== server.id ||
    data.mcp_id !== server.mcpId ||
    !Array.isArray(data.operations) ||
    data.total !== data.operations.length
  )
    throw new Error('Could not verify the complete operation list for this MCP version.');
  const ids = data.operations.map((item) => item.operation_id);
  if (ids.some((id) => typeof id !== 'string' || !id || id.includes('*') || id.includes(',')))
    throw new Error('Fused returned an invalid operation ID.');
  return [...new Set(ids as string[])].sort();
}

class FusedCLIUnavailable extends Error {
  constructor() {
    super('Install fused-cli and make it available on PATH, then reopen Dextana to connect Fused.');
  }
}
...
export class FusedCLI {
  ...
  async initialize() {
    ...
    const legacy = workspace?.userRef;
    let changed = false;
    if (!/^dextana-[0-9a-f-]{36}$/i.test(this.store.state.dextIdentityRef ?? '')) {
      this.store.state.dextIdentityRef = /^dextana-[0-9a-f-]{36}$/i.test(legacy ?? '')
        ? legacy
        : `dextana-${randomUUID()}`;
      changed = true;
    }
    if (workspace?.userRef) {
      delete workspace.userRef;
      changed = true;
    }
    if (changed) {
  ...
  async issue(
    ...
    if (
      this.store.state.fusedWorkspace?.url !== engine ||
      operations.some((value) => !value || value.includes('*') || value.includes(','))
    )
      throw new Error('Review the Fused workspace and exact operation scope.');
    ...
    try {
      return await this.session(async (run) => {
        ...
        try {
          ...
          if (!identity || typeof identity !== 'object' || identity.ok === false)
            throw new Error('Fused identity unavailable.');
          attempted = true;
          const result = parseFusedToken(
            JSON.parse(
              await run([
                'mcp',
                'token',
                ...
                name,
                '--json',
                ...(operations.length ? ['--allow', operations.join(',')] : []),
                '--expires-in',
                FUSED_TOKEN_LIFETIME,
                '--engine-url',
                engine,
                '--no-input',
              ]),
      ...
      throw new Error(
        attempted
          ? 'Fused token creation did not complete reliably. Cleanup was attempted; do not automatically retry issuance. Any issued token expires within 24 hours.'
          : 'Could not verify the Fused account. No token creation was attempted.',
      );
    }
  }
  ...
  async operations(id: string) {
    ...
    if (!workspace || !server) throw new Error('Refresh the connected Fused workspace first.');
    return this.session(async (run) => {
      ...
      if (this.store.state.fusedWorkspace !== workspace)
        throw new Error('The Fused workspace changed. Refresh operations.');
      return parseFusedOperations(result, server);
    });
  }
  async discover() {
    const workspace = this.store.state.fusedWorkspace;
