...
export class DesktopInvocations {
  ...
  constructor(
    ...
    private sessionAllowed: (activity: Activity, work: DesktopWork) => boolean,
  ...
  private async prepare(activity: Activity, tool: DesktopCall) {
    ...
    const grant = plan.operation.mutates
      ? this.sessionGrant(activity, plan.operation.work)
      : 'read';
  ...
  private sessionGrant(activity: Activity, work: DesktopWork) {
    return this.sessionAllowed(activity, work) ? 'session' : 'pending';
  }
  ...
  private async execute(activity: Activity, tool: DesktopCall, signal: AbortSignal) {
    ...
    if (
      prepared.grant === 'session' &&
      !this.sessionAllowed(activity, prepared.plan.operation.work)
    )
