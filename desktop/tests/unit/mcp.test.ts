...
test('disabled tools are absent from discovery and cannot prepare an execution', async () => {
  const { mcp } = setup('disabled');
  expect(mcp.catalog()).toEqual([]);
  expect(() => mcp.prepare('chat', 'call', 'server', 'change', '{}')).toThrow('not enabled');
  const refreshed: unknown[] = [];
  mcp.onCatalogChanged = async () => { refreshed.push(mcp.catalog()); };
  await mcp.setTools('server', { change: 'ask' });
  expect(refreshed).toHaveLength(1);
  expect(mcp.catalog()[0].tools[0].name).toBe('change');
  expect(mcp.prepare('chat', 'enabled-call', 'server', 'change', '{}').requiresApproval).toBe(true);
...
test('session-wide permission allows enabled MCP tools but revocation invalidates pending automatic calls', async () => {
  ...
  store.state.activities[0].permissions = { mcp: true };
  expect(mcp.prepare('chat', 'category-call', 'server', 'change', '{}').requiresApproval).toBe(false);
  expect(mcp.prepare('other-chat', 'other-call', 'server', 'change', '{}').requiresApproval).toBe(true);
  store.state.activities[0].permissions.mcp = false;
