...
async function launchDesktop() {
      ...
      return res.end(
        JSON.stringify({
          capabilities: [
            ...
            'tools',
            'thinking',
            ...(body.model === 'qwen3:8b' ? ['vision'] : []),
          ],
          model_info: {},
          ...(contextWindow === undefined ? {} : { parameters: `num_ctx ${contextWindow}` }),
          template: '',
        }),
      );
    if (req.url !== '/api/chat') {
  ...
  const teachingFixture = join(directory, 'teaching-capture.json');
  await writeFile(
    teachingFixture,
    JSON.stringify([
      {
      ...
      {
        kind: 'app_change',
        application: 'Dext Computer Trial',
        window: 'Contacts',
        role: 'button',
        label: 'Add customer',
        structure: ['Customer form'],
        screenshot: 'data:image/png;base64,iVBORw0KGgo=',
      },
    ]),
  );
  ...
  async function launch() {
    app = await electron.launch({
      ...
      env: {
        ...
        DEXTANA_TEACH_CAPTURE_FIXTURE: teachingFixture,
