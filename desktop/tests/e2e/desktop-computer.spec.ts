...
test('computer discovery routes status through the registered desktop tool', async ({
  ...
  const work = await workspace((body, response) => {
    ...
    if (!results.length) {
      ...
      reply(body, response, 'Computer use status checked.');
    }
    return true;
  });
  try {
    ...
    expect(catalog).toContain('computer.launch');
    expect(receipt).toMatch(/"enabled"\s*:\s*(?:true|false)/);
...
test('computer trial uses shared controls, starts disabled and stays responsive', async ({
  ...
  try {
    ...
    await expect(panel.getByRole('button', { name: 'Check permissions' })).toHaveClass(/dx-button/);
    expect(
      (await panel.getByRole('button', { name: 'Request permissions' }).count()) +
        (await panel.getByText(/Computer use is ready/).count()),
    ).toBe(1);
    await expect(panel.getByLabel('Computer use window')).toHaveCount(0);
    await expect(panel.getByText(/asks before each step by default/)).toBeVisible();
    await expect(panel.getByText(/choose Allow all/).first()).toBeVisible();
    await work
      .app()
