...
test('context stays compact in a narrow conversation and can be minimized and restored', async ({ workspace }) => {
  ...
  try {
    const main = work.page.locator('main');
    const expandedLeft = (await main.boundingBox())!.x;
    await work.page.getByRole('button', { name: 'Collapse chat sidebar', exact: true }).click();
    await expect(work.page.locator('.sidebar')).toBeHidden();
    expect((await main.boundingBox())!.x).toBeLessThan(expandedLeft);
    await work.page.reload();
    await expect(work.page.locator('.sidebar')).toBeHidden();
    await work.page.getByRole('button', { name: 'Show chat sidebar', exact: true }).click();
    await expect(work.page.locator('.sidebar')).toBeVisible();
    ...
    const message = work.page.locator('.message.user').first();
    const time = message.locator('time.message-time');
    await expect(time).toHaveAttribute('datetime', /T/);
    await work.page.getByRole('heading', { name: 'Keep my context separate', exact: true }).hover();
