...
const controlReconnectAttempts = 6;
async function stopBrowserControl(message, notifyServer = true) {
  controlEpoch++;
  ...
  stopping = true;
  try {
    await finishControlStop(session, message, notifyServer);
  } finally {
    stopping = false;
  }
}
async function finishControlStop(session, message, notifyServer) {
  ...
  if (notifyServer) await DextanaTransfer.request(session.target, '/stop', {}).catch(() => {});
...
async function controlLoop(session) {
  while (!session.stopped) {
    try {
      ...
      if (session.reconnectFailures >= controlReconnectAttempts) {
        await stopBrowserControl(
          'Dext was closed or restarted, so browser access ended. Activate browser access again in Dext.',
          false,
        );
        return;
      }
