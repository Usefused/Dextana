...
Choose **Teach Dex** beside **New activity** to demonstrate a workflow across applications, review the captured semantic steps, and replay it as a supervised, versioned skill. Recording starts explicitly, supports pause and typed explanations, redacts sensitive inputs, and keeps the demonstration local until drafting. Taught skills appear under Skills with Run, Edit, and Archive controls plus narrow Draft, Tested successfully, or Needs attention status. See [Teach Dex](docs/teach-dex.md).
...
Chats save all their browser tabs, including pages you navigate to yourself. Closed tabs remain closed after restart. After restarting Dextana, the next approved browser action restores the saved page before continuing. Each chat retains its own persistent cookies and local storage; sites may still require you to sign in again. Unsent forms and page history are not restored. Restoration loads the saved address without replaying previous clicks or submissions. Pages load only when you approve a new browser action.
...
Ask for a budget workbook, contact table, meeting notes, or another work deliverable. Creation shows the destination and a document/table preview before saving. A simple filename saves in **Documents/Dextana**; an absolute path can target an existing folder. Creation never overwrites existing files. To edit an existing file, Dext first reads it, then shows the current and proposed changes for review. UTF-8 text files of any extension support full replacement; Word DOCX supports targeted paragraph-text replacements; Excel XLSX supports existing literal-cell updates while retaining styles and formulas elsewhere; and interactive PDF forms support reviewed text, checkbox, radio, dropdown and list-field updates while remaining fillable. Choose **Apply changes** to save or **Keep original** to decline. Every edit requires approval, including in auto-allow sessions and approved plans. A changed file must be read and reviewed again. Formula cells, protected documents, complex Word content, legacy DOC/XLS, non-form PDFs, XFA/encrypted/signed forms, images and other binary formats require a dedicated editor. XLSX formulas recalculate when opened in Excel. Use **Open** in the top-right **Context** box to launch the result in its default installed app, or **Show in folder** to find it.
...
Excel outputs use formatted headers, frozen header rows, and table filters; formulas in existing workbooks return cached values without recalculation. Supported limits: 5 MB input files, 20 MB expanded workbook data, 20 sheets, 100 columns, 10,000 cells, 500 PDF form fields, and 500,000 text characters. CSV creation accepts one structured table and escapes formula-like text. Word and PDF support text extraction; fillable PDFs without page text still expose their form fields, while scanned PDFs without forms still require OCR. Creation supports XLSX, CSV, TXT and Markdown.
