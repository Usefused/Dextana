...
def files(action: Literal["read", "create", "edit"], path: str, content: str = "", sheets_json: str = "", expected_revision: str = "", edits_json: str = "") -> FileResult:
    ...
    action: read returns UTF-8 text, extracted PDF/DOCX text, or Excel worksheet values. create produces a new
        ...
        DOCX paragraph text, XLSX literal cells, and interactive PDF form fields after the owner reviews every change.
        ...
        Scanned non-form PDFs without a text layer require OCR and cannot be read by this tool; fillable fields are returned separately.
        No shell commands, macros, or formula execution. Legacy .doc/.xls, images and other binary formats cannot be edited.
    ...
    edits_json: For DOCX, XLSX or PDF form editing instead of content, provide 1–100 targeted edits.
        ...
        PDF: [{"field":"Applicant name","value":"Alex Morgan"},{"field":"Consent","value":true}].
        Read returns exact form field names, types, current values, options, and read-only/required flags in form_fields.
        Text fields accept text or null; checkboxes accept booleans or null; radio/dropdown fields accept an existing option
        or null; multiselect lists also accept a text array. The form stays interactive. XFA, encrypted, signed, read-only,
        button, signature, unnamed, duplicate and unsupported fields are not modified. Existing signatures are never invalidated.
